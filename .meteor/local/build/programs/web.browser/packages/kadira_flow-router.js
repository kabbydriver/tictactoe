...
//# sourceMappingURL=local-test:kadira:flow-router/client.browserify.js                                               // 1436
                                                                                                                      // 1437
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
Router.prototype.current = function() {                                                                               // 231
  // We can't trust outside, that's why we clone this                                                                 // 232
  // Anyway, we can't clone the whole object since it has non-jsonable values                                         // 233
  // That's why we clone what's really needed.                                                                        // 234
  var current = _.clone(this._current);                                                                               // 235
  current.queryParams = EJSON.clone(current.queryParams);                                                             // 236
  current.params = EJSON.clone(current.params);                                                                       // 237
  return current;                                                                                                     // 238
};                                                                                                                    // 239
                                                                                                                      // 240
// Implementing Reactive APIs                                                                                         // 241
var reactiveApis = [                                                                                                  // 242
  'getParam', 'getQueryParam',                                                                                        // 243
  'getRouteName', 'watchPathChange'                                                                                   // 244
];                                                                                                                    // 245
reactiveApis.forEach(function(api) {                                                                                  // 246
  Router.prototype[api] = function(arg1) {                                                                            // 247
    // when this is calling, there may not be any route initiated                                                     // 248
    // so we need to handle it                                                                                        // 249
    var currentRoute = this._current.route;                                                                           // 250
    if(!currentRoute) {                                                                                               // 251
      this._onEveryPath.depend();                                                                                     // 252
      return;                                                                                                         // 253
    }                                                                                                                 // 254
                                                                                                                      // 255
    // currently, there is only one argument. If we've more let's add more args                                       // 256
    // this is not clean code, but better in performance                                                              // 257
    return currentRoute[api].call(currentRoute, arg1);                                                                // 258
  };                                                                                                                  // 259
});                                                                                                                   // 260
                                                                                                                      // 261
Router.prototype.subsReady = function() {                                                                             // 262
  var callback = null;                                                                                                // 263
  var args = _.toArray(arguments);                                                                                    // 264
                                                                                                                      // 265
  if (typeof _.last(args) === "function") {                                                                           // 266
    callback = args.pop();                                                                                            // 267
  }                                                                                                                   // 268
                                                                                                                      // 269
  var currentRoute = this.current().route;                                                                            // 270
  var globalRoute = this._globalRoute;                                                                                // 271
                                                                                                                      // 272
  // we need to depend for every route change and                                                                     // 273
  // rerun subscriptions to check the ready state                                                                     // 274
  this._onEveryPath.depend();                                                                                         // 275
                                                                                                                      // 276
  if(!currentRoute) {                                                                                                 // 277
    return false;                                                                                                     // 278
  }                                                                                                                   // 279
                                                                                                                      // 280
  var subscriptions;                                                                                                  // 281
  if(args.length === 0) {                                                                                             // 282
    subscriptions = _.values(globalRoute.getAllSubscriptions());                                                      // 283
    subscriptions = subscriptions.concat(_.values(currentRoute.getAllSubscriptions()));                               // 284
  } else {                                                                                                            // 285
    subscriptions = _.map(args, function(subName) {                                                                   // 286
      return globalRoute.getSubscription(subName) || currentRoute.getSubscription(subName);                           // 287
    });                                                                                                               // 288
  }                                                                                                                   // 289
                                                                                                                      // 290
  var isReady = function() {                                                                                          // 291
    var ready =  _.every(subscriptions, function(sub) {                                                               // 292
      return sub && sub.ready();                                                                                      // 293
    });                                                                                                               // 294
                                                                                                                      // 295
    return ready;                                                                                                     // 296
  };                                                                                                                  // 297
                                                                                                                      // 298
  if (callback) {                                                                                                     // 299
    Tracker.autorun(function(c) {                                                                                     // 300
      if (isReady()) {                                                                                                // 301
        callback();                                                                                                   // 302
        c.stop();                                                                                                     // 303
      }                                                                                                               // 304
    ...
    return isReady();                                                                                                 // 307
                                                                                                                      ...
                                                                                                                      // 310
Router.prototype.withReplaceState = function(fn) {                                                                    // 311
  return this.env.replaceState.withValue(true, fn);                                                                   // 312
};                                                                                                                    // 313
                                                                                                                      // 314
Router.prototype.withTrailingSlash = function(fn) {                                                                   // 315
  return this.env.trailingSlash.withValue(true, fn);                                                                  // 316
};                                                                                                                    // 317
                                                                                                                      // 318
Router.prototype._notfoundRoute = function(context) {                                                                 // 319
  this._current = {                                                                                                   // 320
    path: context.path,                                                                                               // 321
    context: context,                                                                                                 // 322
    params: [],                                                                                                       // 323
    queryParams: {},                                                                                                  // 324
  };                                                                                                                  // 325
                                                                                                                      // 326
  // XXX this.notfound kept for backwards compatibility                                                               // 327
  this.notFound = this.notFound || this.notfound;                                                                     // 328
  if(!this.notFound) {                                                                                                // 329
    console.error("There is no route for the path:", context.path);                                                   // 330
    return;                                                                                                           // 331
  }                                                                                                                   // 332
                                                                                                                      // 333
  this._current.route = new Route(this, "*", this.notFound);                                                          // 334
  this._invalidateTracker();                                                                                          // 335
};                                                                                                                    // 336
                                                                                                                      // 337
Router.prototype.initialize = function(options) {                                                                     // 338
  options = options || {};                                                                                            // 339
                                                                                                                      // 340
  if(this._initialized) {                                                                                             // 341
    throw new Error("FlowRouter is already initialized");                                                             // 342
  }                                                                                                                   // 343
                                                                                                                      // 344
  var self = this;                                                                                                    // 345
  this._updateCallbacks();                                                                                            // 346
                                                                                                                      // 347
  // Implementing idempotent routing                                                                                  // 348
  // by overriding page.js`s "show" method.                                                                           // 349
  // Why?                                                                                                             // 350
  // It is impossible to bypass exit triggers,                                                                        // 351
  // becuase they execute before the handler and                                                                      // 352
  // can not know what the next path is, inside exit trigger.                                                         // 353
  //                                                                                                                  // 354
  // we need override both show, replace to make this work                                                            // 355
  // since we use redirect when we are talking about withReplaceState                                                 // 356
  _.each(['show', 'replace'], function(fnName) {                                                                      // 357
    var original = self._page[fnName];                                                                                // 358
    self._page[fnName] = function(path, state, dispatch, push) {                                                      // 359
      var reload = self.env.reload.get();                                                                             // 360
      if (!reload && self._current.path === path) {                                                                   // 361
        return;                                                                                                       // 362
      }                                                                                                               // 363
                                                                                                                      // 364
      original.call(this, path, state, dispatch, push);                                                               // 365
    };                                                                                                                // 366
  });                                                                                                                 // 367
                                                                                                                      // 368
  // this is very ugly part of pagejs and it does decoding few times                                                  // 369
  // in unpredicatable manner. See #168                                                                               // 370
  // this is the default behaviour and we need keep it like that                                                      // 371
  // we are doing a hack. see .path()                                                                                 // 372
  this._page({decodeURLComponents: true, hashbang: !!options.hashbang});                                              // 373
  this._initialized = true;                                                                                           // 374
};                                                                                                                    // 375
                                                                                                                      // 376
Router.prototype._buildTracker = function() {                                                                         // 377
  var self = this;                                                                                                    // 378
                                                                                                                      // 379
  // main autorun function                                                                                            // 380
  var tracker = Tracker.autorun(function () {                                                                         // 381
    if(!self._current || !self._current.route) {                                                                      // 382
      return;                                                                                                         // 383
    }                                                                                                                 // 384
                                                                                                                      // 385
    // see the definition of `this._processingContexts`                                                               // 386
    var currentContext = self._current;                                                                               // 387
    var route = currentContext.route;                                                                                 // 388
    var path = currentContext.path;                                                                                   // 389
                                                                                                                      // 390
    if(self.safeToRun === 0) {                                                                                        // 391
      var message =                                                                                                   // 392
        "You can't use reactive data sources like Session" +                                                          // 393
        " inside the `.subscriptions` method!";                                                                       // 394
      throw new Error(message);                                                                                       // 395
    }                                                                                                                 // 396
                                                                                                                      // 397
    // We need to run subscriptions inside a Tracker                                                                  // 398
    // to stop subs when switching between routes                                                                     // 399
    // But we don't need to run this tracker with                                                                     // 400
    // other reactive changes inside the .subscription method                                                         // 401
    // We tackle this with the `safeToRun` variable                                                                   // 402
    self._globalRoute.clearSubscriptions();                                                                           // 403
    self.subscriptions.call(self._globalRoute, path);                                                                 // 404
    route.callSubscriptions(currentContext);                                                                          // 405
                                                                                                                      // 406
    // otherwise, computations inside action will trigger to re-run                                                   // 407
    // this computation. which we do not need.                                                                        // 408
    Tracker.nonreactive(function() {                                                                                  // 409
      var isRouteChange = currentContext.oldRoute !== currentContext.route;                                           // 410
      var isFirstRoute = !currentContext.oldRoute;                                                                    // 411
      // first route is not a route change                                                                            // 412
      if(isFirstRoute) {                                                                                              // 413
        isRouteChange = false;                                                                                        // 414
      }                                                                                                               // 415
                                                                                                                      // 416
      // Clear oldRouteChain just before calling the action                                                           // 417
      // We still need to get a copy of the oldestRoute first                                                         // 418
      // It's very important to get the oldest route and registerRouteClose() it                                      // 419
      // See: https://github.com/kadirahq/flow-router/issues/314                                                      // 420
      var oldestRoute = self._oldRouteChain[0];                                                                       // 421
      self._oldRouteChain = [];                                                                                       // 422
                                                                                                                      // 423
      currentContext.route.registerRouteChange(currentContext, isRouteChange);                                        // 424
      route.callAction(currentContext);                                                                               // 425
                                                                                                                      // 426
      Tracker.afterFlush(function() {                                                                                 // 427
        self._onEveryPath.changed();                                                                                  // 428
        if(isRouteChange) {                                                                                           // 429
          // We need to trigger that route (definition itself) has changed.                                           // 430
          // So, we need to re-run all the register callbacks to current route                                        // 431
          // This is pretty important, otherwise tracker                                                              // 432
          // can't identify new route's items                                                                         // 433
                                                                                                                      // 434
          // We also need to afterFlush, otherwise this will re-run                                                   // 435
          // helpers on templates which are marked for destroying                                                     // 436
          if(oldestRoute) {                                                                                           // 437
            oldestRoute.registerRouteClose();                                                                         // 438
          }                                                                                                           // 439
        }                                                                                                             // 440
      });                                                                                                             // 441
    });                                                                                                               // 442
  ...
  return tracker;                                                                                                     // 447
                                                                                                                      ...
                                                                                                                      // 449
Router.prototype._invalidateTracker = function() {                                                                    // 450
  var self = this;                                                                                                    // 451
  this.safeToRun++;                                                                                                   // 452
  this._tracker.invalidate();                                                                                         // 453
  // After the invalidation we need to flush to make changes imediately                                               // 454
  // otherwise, we have face some issues context mix-maches and so on.                                                // 455
  // But there are some cases we can't flush. So we need to ready for that.                                           // 456
                                                                                                                      // 457
  // we clearly know, we can't flush inside an autorun                                                                // 458
  // this may leads some issues on flow-routing                                                                       // 459
  // we may need to do some warning                                                                                   // 460
  if(!Tracker.currentComputation) {                                                                                   // 461
    // Still there are some cases where we can't flush                                                                // 462
    //  eg:- when there is a flush currently                                                                          // 463
    // But we've no public API or hacks to get that state                                                             // 464
    // So, this is the only solution                                                                                  // 465
    try {                                                                                                             // 466
      Tracker.flush();                                                                                                // 467
    } catch(ex) {                                                                                                     // 468
      // only handling "while flushing" errors                                                                        // 469
      if(!/Tracker\.flush while flushing/.test(ex.message)) {                                                         // 470
        return;                                                                                                       // 471
      }                                                                                                               // 472
                                                                                                                      // 473
      // XXX: fix this with a proper solution by removing subscription mgt.                                           // 474
      // from the router. Then we don't need to run invalidate using a tracker                                        // 475
                                                                                                                      // 476
      // this happens when we are trying to invoke a route change                                                     // 477
      // with inside a route chnage. (eg:- Template.onCreated)                                                        // 478
      // Since we use page.js and tracker, we don't have much control                                                 // 479
      // over this process.                                                                                           // 480
      // only solution is to defer route execution.                                                                   // 481
                                                                                                                      // 482
      // It's possible to have more than one path want to defer                                                       // 483
      // But, we only need to pick the last one.                                                                      // 484
      // self._nextPath = self._current.path;                                                                         // 485
      Meteor.defer(function() {                                                                                       // 486
        var path = self._nextPath;                                                                                    // 487
        if(!path) {                                                                                                   // 488
          return;                                                                                                     // 489
        }                                                                                                             // 490
                                                                                                                      // 491
        delete self._nextPath;                                                                                        // 492
        self.env.reload.withValue(true, function() {                                                                  // 493
          self.go(path);                                                                                              // 494
        });                                                                                                           // 495
      });                                                                                                             // 496
    }                                                                                                                 // 497
                                                                                                                      ...
                                                                                                                      // 500
Router.prototype._updateCallbacks = function () {                                                                     // 501
  var self = this;                                                                                                    // 502
                                                                                                                      // 503
  self._page.callbacks = [];                                                                                          // 504
  self._page.exits = [];                                                                                              // 505
                                                                                                                      // 506
  _.each(self._routes, function(route) {                                                                              // 507
    self._page(route.pathDef, route._actionHandle);                                                                   // 508
    self._page.exit(route.pathDef, route._exitHandle);                                                                // 509
  });                                                                                                                 // 510
                                                                                                                      // 511
  self._page("*", function(context) {                                                                                 // 512
    self._notfoundRoute(context);                                                                                     // 513
  });                                                                                                                 // 514
};                                                                                                                    // 515
                                                                                                                      // 516
Router.prototype._initTriggersAPI = function() {                                                                      // 517
  var self = this;                                                                                                    // 518
  this.triggers = {                                                                                                   // 519
    enter: function(triggers, filter) {                                                                               // 520
      triggers = Triggers.applyFilters(triggers, filter);                                                             // 521
      if(triggers.length) {                                                                                           // 522
        self._triggersEnter = self._triggersEnter.concat(triggers);                                                   // 523
      }                                                                                                               // 524
    },                                                                                                                // 525
                                                                                                                      // 526
    exit: function(triggers, filter) {                                                                                // 527
      triggers = Triggers.applyFilters(triggers, filter);                                                             // 528
      if(triggers.length) {                                                                                           // 529
        self._triggersExit = self._triggersExit.concat(triggers);                                                     // 530
      }                                                                                                               // 531
    }                                                                                                                 // 532
  };                                                                                                                  // 533
};                                                                                                                    // 534
                                                                                                                      // 535
Router.prototype.wait = function() {                                                                                  // 536
  if(this._initialized) {                                                                                             // 537
    throw new Error("can't wait after FlowRouter has been initialized");                                              // 538
  }                                                                                                                   // 539
                                                                                                                      // 540
  this._askedToWait = true;                                                                                           // 541
};                                                                                                                    // 542
                                                                                                                      // 543
Router.prototype.onRouteRegister = function(cb) {                                                                     // 544
  this._onRouteCallbacks.push(cb);                                                                                    // 545
};                                                                                                                    // 546
                                                                                                                      // 547
Router.prototype._triggerRouteRegister = function(currentRoute) {                                                     // 548
  // We should only need to send a safe set of fields on the route                                                    // 549
  // object.                                                                                                          // 550
  // This is not to hide what's inside the route object, but to show                                                  // 551
  // these are the public APIs                                                                                        // 552
  var routePublicApi = _.pick(currentRoute, 'name', 'pathDef', 'path');                                               // 553
  var omittingOptionFields = [                                                                                        // 554
    'triggersEnter', 'triggersExit', 'action', 'subscriptions', 'name'                                                // 555
  ];                                                                                                                  // 556
  routePublicApi.options = _.omit(currentRoute.options, omittingOptionFields);                                        // 557
                                                                                                                      // 558
  _.each(this._onRouteCallbacks, function(cb) {                                                                       // 559
    cb(routePublicApi);                                                                                               // 560
  });                                                                                                                 // 561
};                                                                                                                    // 562
                                                                                                                      // 563
Router.prototype._page = page;                                                                                        // 564
Router.prototype._qs = qs;                                                                                            // 565
                                                                                                                      // 566
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
